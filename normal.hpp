...
public:
    void Run(int run, int MAX_NFE, int pop, int DIM, int FunctionNumber, int FunctionTransform)
    {
        ...
        while (r < run)
        {
            ...
            for (int i = 0; i < pop; i++)
            {
                RANDOM_INI(DIM, i, PSO_inf.Particle, max, min);
                RANDOM_INI(DIM, i, PSO_inf.Velocity, 0.02 * max, 0.02 * min);
            }
...
private:
    void RANDOM_INI(int DIM, int index, d2d &arr, int Max, int Min)
    {
    ...
    {
        double delta1 = 0.05 * Delta_MAX;
        double delta2 = 0.10 * Delta_MAX;
        double delta3 = 0.15 * Delta_MAX;
        ...
        for (int i = 0; i < pop; i++)
        {
            ...
            //delta
            //low
            if (PSO_inf.delta[i] >= 0.0 && PSO_inf.delta[i] < delta1)
                Fuzzy_coef.Delta_coef[i][0] = 1.0;
            else if (PSO_inf.delta[i] >= delta1 && PSO_inf.delta[i] < delta2)
                Fuzzy_coef.Delta_coef[i][0] = (delta2 - PSO_inf.delta[i]) / (delta2 - delta1);
            else if (PSO_inf.delta[i] >= delta2 && PSO_inf.delta[i] <= 1.0)
            ...
            if (PSO_inf.delta[i] >= 0.0 && PSO_inf.delta[i] < delta1)
                ...
                Fuzzy_coef.Delta_coef[i][1] = (PSO_inf.delta[i] - delta1) / (delta2 - delta1);
            else if (PSO_inf.delta[i] >= delta2 && PSO_inf.delta[i] < delta3)
                Fuzzy_coef.Delta_coef[i][1] = (delta3 - PSO_inf.delta[i]) / (delta3 - delta2);
            else if (PSO_inf.delta[i] >= delta3 && PSO_inf.delta[i] <= 1.0)
                Fuzzy_coef.Delta_coef[i][1] = 0.0;
            //high
            if (PSO_inf.delta[i] >= 0.0 && PSO_inf.delta[i] < delta2)
    ...
    void Update_Velocity(int pop, int DIM)
    {
        for (int i = 0; i < pop; i++)
        {
            for (int j = 0; j < DIM; j++)
            {
                ...
                PSO_inf.Velocity[i][j] = Fuzzy_coef.Inertia[i] * PSO_inf.Velocity[i][j] + r1 * Fuzzy_coef.Social[i] * (Current_inf.Current_Best_Coordinate[j] - PSO_inf.Particle[i][j]) + r2 * Fuzzy_coef.Cognitive[i] * (Personal_inf.Personal_Best_Coordinate[i][j] - PSO_inf.Particle[i][j]);
    ...
    void Evaluation(int pop, int DIM, int F, int F_T)
    {
        for (int i = 0; i < pop; i++)
        {
            ...
            if (PSO_inf.Objective[i] <= Current_inf.Current_Best_Value)
            {
                ...
                Current_inf.Current_Best_Coordinate = PSO_inf.Particle[i];
            }
            if (NFE <= pop && PSO_inf.Objective[i] >= Current_inf.Current_Worst_Value)
            {
